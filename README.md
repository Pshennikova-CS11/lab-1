...
Ініціалізація таблиць також виконується автоматично при старті сервера через CREATE TABLE IF NOT EXISTS, тому 
вручну створювати таблиці не потрібно.
...
Для пов’язаних записів використано FOREIGN KEY та ON DELETE CASCADE, тобто при видаленні ресурсу або користувача 
пов’язані оцінки видаляються автоматично.
...
Frontend працює з DTO, які повертає backend. Тому backend не повинен перейменовувати або видаляти поля, які 
вже використовуються у frontend-частині.
...
Наприклад, не можна перейменувати title у name, url у link, id у resourceId, тому що frontend уже використовує 
ці поля для відображення таблиці, редагування та видалення записів.
...
Поля code, message і details не можна перейменовувати, тому що frontend використовує їх для показу зрозумілих 
...
У цьому файлі зібрана логіка HTTP-запитів до backend. Frontend-код у app.js не виконує fetch() напряму, 
а використовує готові API-методи.
...
Frontend працює на порту `5500`, а backend — на порту `3000`, тому для коректної взаємодії між ними необхідне 
налаштування CORS.
...
Після створення, редагування або видалення frontend повторно завантажує актуальні дані з backend. Це запобігає 
дублюванню або появі “фантомних” елементів у таблицях.
...
Для запитів встановлено таймаут 15 секунд. Якщо backend не відповідає довше, запит скасовується через AbortController, 
а frontend показує користувачу зрозуміле повідомлення.


Лабораторна робота №5. Уразливості і захист
5.1. Сценарій А — SQL Injection
Для сценарію SQL Injection було перевірено роботу з сутністю Resources. До виправлення частина SQL-запитів формувалася 
через вставляння користувацьких значень безпосередньо в SQL-рядок. Такий підхід є небезпечним, оскільки введення 
користувача може змінити структуру SQL-запиту або спричинити SQL-помилку.
Для виправлення було змінено роботу з базою даних. У файлі dbClient.js функції all(), get() і run() були доповнені 
підтримкою параметрів. Після цього запити у resources.repository.js були переписані з використанням параметризованих 
запитів через placeholders ?. Значення більше не вставляються напряму в SQL-рядок, а передаються окремим масивом параметрів.

Також у resources.service.js фільтри за author і type були переведені на параметризовану передачу значень. 
Для полів сортування використано allowlist, тобто дозволяються лише заздалегідь визначені значення.

Для перевірки було виконано звичайний запит фільтрації за автором, наприклад:
GET http://localhost:3000/api/v1/resources?author=Шевченко

Також було перевірено введення з апострофом:
GET http://localhost:3000/api/v1/resources?author=O'Reilly

Після виправлення апостроф не спричиняє SQL-помилку. Для перевірки SQL Injection було використано введення:
' OR '1'='1
Після параметризації такий ввід не змінює логіку SQL-запиту і не повертає всі записи, а обробляється як звичайний текст.

5.2. Сценарій В — Broken Access Control / IDOR
Для сценарію IDOR було обрано сутність Comments, оскільки кожен коментар має поле userId. Це поле використовується 
як власник ресурсу, тобто як еквівалент ownerUserId.
До виправлення доступ до конкретного коментаря виконувався лише за id. Це означало, що користувач потенційно міг 
підставити чужий id у URL-запиті та отримати, змінити або видалити чужий коментар.

Для виправлення було реалізовано спрощене визначення поточного користувача через HTTP-заголовок:
X-Demo-UserId:
Було створено middleware demoAuth, яке перевіряє наявність заголовка X-Demo-UserId, коректність id користувача 
та існування такого користувача в таблиці Users. Якщо заголовок відсутній або користувач не існує, backend 
повертає 401 Unauthorized.

Після цього у routes для comments було підключено demoAuth до операцій читання конкретного коментаря, створення, 
оновлення та видалення. У controller поточний користувач передається в service через req.user.id.

У comments.repository.js запити до конкретного коментаря були змінені так, щоб перевіряти не тільки id коментаря, 
а й власника: WHERE id = ? AND userId = ?

Таким чином, користувач може працювати тільки з тими коментарями, які належать саме йому.
Для перевірки було виконано такі запити:
GET http://localhost:3000/api/v1/comments/6 без X-Demo-UserId — результат 401 Unauthorized.
GET http://localhost:3000/api/v1/comments/6 з X-Demo-UserId: 99999 — результат 401 Unauthorized, оскільки такого користувача немає.
GET http://localhost:3000/api/v1/comments/6 з X-Demo-UserId: 1 — результат 200 OK, якщо коментар належить користувачу 1.
GET http://localhost:3000/api/v1/comments/6 з X-Demo-UserId: 2 — результат 404 Not Found, якщо коментар належить іншому користувачу.

5.3. Сценарій Б – XSS (відбитий або збережений)
Для сценарію XSS було перевірено відображення користувацьких коментарів на frontend.
До виправлення коментарі виводилися через innerHTML, а текст користувача вставлявся безпосередньо в HTML-шаблон.
Такий підхід є небезпечним, оскільки введений користувачем HTML або JavaScript-код може бути інтерпретований
браузером як розмітка або скрипт.

Для відтворення було використано HTML/JS-ввід у полі коментаря, наприклад: <img src=x onerror=alert('XSS')>
У небезпечній версії такий ввід міг бути оброблений браузером як HTML-код.

Для виправлення у файлі app.js рендеринг коментарів було переписано через DOM API. Замість вставляння даних користувача
через innerHTML використовується textContent. Для цього створено функцію createTextCell(value), яка створює елемент td
і записує значення через textContent.
Це виправлення не забороняє користувачу вводити символи <, > або інший текст, а безпечно відображає введення як
звичайний текст. Такий підхід є правильним, тому що захист виконується саме на етапі відображення даних у браузері.

Після виправлення введення: <img src=x onerror=alert('XSS')>
відображається у таблиці як звичайний текст. JavaScript-код не виконується, alert не з’являється, а структура сторінки не змінюється.

5.4. Зміни у CORS та frontend
Після додавання заголовка X-Demo-UserId виникла необхідність оновити CORS-налаштування backend. У allowedHeaders 
було додано X-Demo-UserId, щоб браузер дозволяв frontend надсилати цей заголовок у запитах до API.

У frontend-частині для операцій із коментарями було додано передачу X-Demo-UserId. 
Значення поточного користувача береться з поля вибору користувача у формі коментаря. 
При цьому userId більше не використовується як джерело прав доступу з body-запиту. 
У body передаються тільки дані коментаря, а поточний користувач визначається через заголовок.
Це дозволило зберегти роботу CRUD-функціоналу для коментарів після додавання захисту.
